...
GENERAL DESCRIPTION:
    ...
    1) File used to generate the graph with multiple mid price trajectories. 
...
if __name__ == "__main__":
    ...
    nb_graphs = 14
    for param in range(nb_graphs):
        print(f"Starting simulation {param+1}/14")

        file_name = f'mid_price_path_{param+1}' # if generating mid price trajectories figure for the paper 
        # file_name = 'lob_trades_fit_propagator'  # if generating data to fit propagator model
        ...
        df.to_parquet(
            f"mid_price_simulations/{file_name}.parquet",
            engine="pyarrow",
            compression="zstd",    
            index=False
        )
