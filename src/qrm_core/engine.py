...
def simulate_QRM_jit(time: float,
    ...
    ## HERE
    count = 1
    p_refs = np.cumsum(slope * np.ones(max_events, np.float64)) + p_ref
    times   = np.cumsum(np.ones(max_events, np.float64)) + time
    p_mids  = p_refs.copy()
    sides   = np.ones(max_events, np.int32)
    depths  = np.ones(max_events, np.int32)
    events  = np.ones(max_events, np.int32)
    redrawns = np.ones(max_events, np.int32)
    states  = 100 * np.ones((max_events, 2*K), np.int32)
