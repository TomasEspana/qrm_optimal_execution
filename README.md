...
- `depth`: price level **relative to `p_ref`** on that side (`1` = 0.5 ticks away from `p_ref`, `2` = 1.5 ticks away from `p_ref`, …).
...
Run `src/qrm_core/invariant_distribution.py` which will create and save the `.npy` file of the invariant distribution in the folder `calibration_data`. 
